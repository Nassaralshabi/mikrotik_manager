...
class _DatabaseCleanupScreenState extends State<DatabaseCleanupScreen> {
  ...
  Future<void> _loadDatabaseInfo() async {
    ...
    setState(() {
      _stepLogs.add('[${DateTime.now().toString().split('.')[0]}] بدء إنشاء النسخة الاحتياطية...');
    });
    ...
    try {
      ...
      setState(() {
        _backupCreated = true;
        _stepLogs.add('[${DateTime.now().toString().split('.')[0]}] ✅ تم إنشاء النسخة الاحتياطية: $backupName');
      });
      ...
      setState(() {
        _stepLogs.add('[${DateTime.now().toString().split('.')[0]}] ❌ فشل إنشاء النسخة الاحتياطية: $e');
      });
      if (mounted) {
  ...
  Future<void> _startCleanup() async {
    setState(() {
      ...
      _currentStep = 0;
      _stepLogs.clear();
      _stepLogs.add('[${DateTime.now().toString().split('.')[0]}] بدء عملية تنظيف قاعدة البيانات...');
    });
    ...
    for (int i = 0; i < _steps.length; i++) {
      ...
      setState(() {
        _currentStep = i;
        _stepLogs.add('[${DateTime.now().toString().split('.')[0]}] تنفيذ: ${_steps[i].title}');
      });
      ...
      if (_steps[i].delay > 0) {
        setState(() {
          _stepLogs.add('[${DateTime.now().toString().split('.')[0]}] انتظار ${_steps[i].delay} ثانية...');
        });
        await Future.delayed(Duration(seconds: _steps[i].delay));
      }

      setState(() {
        _stepLogs.add('[${DateTime.now().toString().split('.')[0]}] ✅ مكتمل: ${_steps[i].title}');
      });
    }
    ...
    setState(() {
      ...
      _stepLogs.add('[${DateTime.now().toString().split('.')[0]}] 🎉 اكتملت عملية التنظيف بنجاح!');
    });
  ...
  Future<void> _executeStep(CleanupStep step) async {
    ...
    try {
      ...
      if (step.isReboot) {
        ...
        setState(() {
          _stepLogs.add('[${DateTime.now().toString().split('.')[0]}] 🔄 تم إرسال أمر إعادة التشغيل...');
        });
        return;
      }
      ...
      setState(() {
        _stepLogs.add('[${DateTime.now().toString().split('.')[0]}] النتيجة: ${response.length} استجابة');
      });

    } catch (e) {
      setState(() {
        _stepLogs.add('[${DateTime.now().toString().split('.')[0]}] ⚠️ خطأ: $e');
      });
    } finally {
      client?.close();
    }
  ...
  void _stopCleanup() {
    setState(() {
      _isRunning = false;
      _stepLogs.add('[${DateTime.now().toString().split('.')[0]}] ⏹️ تم إيقاف العملية من قبل المستخدم');
    });
  }
}
